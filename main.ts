...
function create_enemy()
{
    ...
    if(level%10 > 4)
    {
        for(let value of tiles_location)
        {
...
{   
    ...
    for(let enemy of jk_array)
    {
        ...
        {
            if (Math.abs(enemy.x - rowa.entity.x) < 20)
            {
                if (enemy.x - rowa.entity.x < -5)
                {
                    enemy.vx = 10*5
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, rowa.lose())
...
function ()
{
    ...
    if(level%10 > 4)
    {
        controller.moveSprite(rowa.entity,160,0)
    }
})
